    ...
    <ng-container *transloco="let t">
      ...
      >
        <div class="flex items-center justify-between px-6 py-4 border-b border-neutral-200 dark:border-neutral-800">
          ...
          >
            @if (category()) { {{ t('categories.form.titleEdit') }} } @else { {{ t('categories.form.titleNew') }} }
          </h2>
          <button
            type="button"
            (click)="cancel()"
            [attr.aria-label]="t('categories.form.close')"
        ...
        <form [formGroup]="form" (ngSubmit)="submit()" novalidate class="px-6 py-4 flex flex-col gap-4">
          ...
          <div class="flex justify-end gap-3 pt-1">
            ...
            >
              @if (loading()) { {{ t('categories.form.saving') }} } @else { {{ t('categories.form.save') }} }
            </button>
          </div>
        </form>
      </dialog>
...
export class CategoryFormComponent {
  ...
  nameError(): string | null {
    ...
    if (ctrl.hasError('maxlength')) return this.t.translate('validation.nameMaxLength100');
    return null;
  }
  ...
  submit(): void {
    ...
    operation$.subscribe({
      ...
      error: err => {
        this.loading.set(false);
        this.error.set(err?.error?.message ?? this.t.translate('categories.form.genericError'));
      },
    });
  }
}
