...
@Component({
  ...
  imports: [DatePipe, CategoryFormComponent, TranslocoModule],
  template: `
    <ng-container *transloco="let t">
      <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-xl font-semibold text-neutral-900 dark:text-neutral-100">{{ t('categories.title') }}</h1>
            <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
              @if (auth.isAdmin()) {
                {{ t('categories.subtitleAdmin') }}
              } @else {
                {{ t('categories.subtitleUser') }}
              }
            </p>
          </div>
          @if (auth.isAdmin()) {
        ...
        @if (deleteError()) {
          ...
          >
            ...
            <button
              ...
              [attr.aria-label]="t('categories.closeAlert')"
        ...
        @if (categoryService.loading()) {
          ...
          @if (categoryService.categories().length === 0) {
            <div class="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-6 py-12 text-center">
              <p class="text-sm text-neutral-500 dark:text-neutral-400">{{ t('categories.empty') }}</p>
              @if (auth.isAdmin()) {
                <button
                  type="button"
                  (click)="openCreate()"
                        ...
                        <td class="px-6 py-4 text-right">
                          @if (pendingDeleteId() === cat.id) {
                            <div class="inline-flex items-center gap-2">
                              <span class="text-xs text-neutral-600 dark:text-neutral-400">{{ t('categories.deleteConfirm') }}</span>
                              <button
                                type="button"
                              ...
                              >
                                @if (mutatingId() === cat.id) { … } @else { {{ t('common.yes') }} }
                              </button>
                              <button
                                type="button"
  ...
  protected confirmDelete(id: number): void {
    ...
    this.categoryService.remove(id).subscribe({
      ...
      error: err => {
        this.mutatingId.set(null);
        this.pendingDeleteId.set(null);
        this.deleteError.set(err?.error?.message ?? this.t.translate('categories.deleteError'));
      },
    });
  }
}
