    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Gerencie os usuários do sistema.</p>
        </div>
        <button
          type="button"
          (click)="openCreate()"
      ...
      @if (userService.loading()) {
        ...
        @if (userService.users().length === 0) {
          <div class="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-6 py-12 text-center">
            <p class="text-sm text-neutral-500 dark:text-neutral-400">Nenhum usuário cadastrado.</p>
            <button
              type="button"
              (click)="openCreate()"
          ...
          <div class="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 overflow-hidden">
            <table class="w-full text-sm">
              <thead class="bg-neutral-50 dark:bg-neutral-800/50">
                <tr>
                  <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">Nome</th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">E-mail</th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">Perfil</th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">Criado em</th>
                  <th class="px-6 py-3 text-right text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">Ações</th>
                </tr>
              </thead>
                    ...
                    <td class="px-6 py-4 text-right">
                      @if (pendingDeleteId() === user.id) {
                        <div class="inline-flex items-center gap-2">
                          ...
                          >
                            @if (mutatingId() === user.id) { … } @else { Sim }
                          </button>
                          <button
                            type="button"
  ...
  protected confirmDelete(id: number): void {
    ...
    this.userService.remove(id).subscribe({
      ...
      error: err => {
        this.mutatingId.set(null);
        this.pendingDeleteId.set(null);
        this.deleteError.set(err?.error?.message ?? 'Erro ao excluir usuário. Tente novamente.');
      },
    });
  }
}
