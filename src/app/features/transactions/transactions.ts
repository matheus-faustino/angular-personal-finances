  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-semibold text-neutral-900 dark:text-neutral-100">Transações</h1>
          <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
            @if (auth.isAdmin()) {
              Visualize e filtre as transações dos usuários.
            } @else {
      ...
      @if (transactionService.loading()) {
        ...
        @if (transactionService.transactions().length === 0) {
          <div class="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-6 py-12 text-center">
            <p class="text-sm text-neutral-500 dark:text-neutral-400">Nenhuma transação encontrada.</p>
            @if (!auth.isAdmin()) {
              <button
                type="button"
                (click)="openCreate()"
